...
dot t1@(Dense v1 [s1]) t2@(Dense v2 [s2]) | s1 == 1 && s2 == 1 = tensorMulitply t1 t2 --0-D 0-D
                                          | otherwise = inner t1 t2 --1-D 1-D
dot t1@(Dense v1 [s11:s12]) t2@(Dense v2 [s21:s22]) = matmul t1 t2 --2-D 2-D
dot t1@(Dense v1 s1) t2@(Dense v2 [s2]) = -- Sum over last axis of a and b
dot t1@(Dense v1 s1) t2@(Dense v2 s2) = -- Sum over last axis of a and second-to-last axis of b
