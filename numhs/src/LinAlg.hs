...
getDim :: Default a => Tensor a -- ^`Tensor` for which the dimension should be found.
...
getRow2D :: (Num a, Default a) => Tensor a  -> Int -> Either Error [a]
...
getColumn2D :: (Num a, Default a) => Tensor a -> Int -> Either Error [a]
...
tensorDotIndex :: (Num a, Default a) => Tensor a -> Tensor a -> Int -> Int -> a
...
matmul :: (Num a, Default a) => Tensor a -> Tensor a -> Either Error (Tensor a)
